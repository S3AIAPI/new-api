...
export function ChannelMutateDrawer({
  ...
  const openAICompatibilityFields = currentType === 1 && (
    <>
      <FormField
        control={form.control}
        ...
        render={({ field }) => (
          <FormItem className='flex items-center justify-between gap-3 px-4 py-3'>
            <div className='space-y-0.5'>
              <FormLabel>{t('Use Responses API Only')}</FormLabel>
              <FormDescription>
                {t(
      ...
      <FormField
        ...
        render={({ field }) => (
          <FormItem className='flex items-center justify-between gap-3 px-4 py-3'>
            <div className='space-y-0.5'>
              <FormLabel>{t('Fake Non-Stream Support')}</FormLabel>
              <FormDescription>
                {t(
  ...
  const remoteCompactFields = (
    <FormField
      ...
      render={({ field }) => (
        <FormItem className='flex items-center justify-between gap-3 px-4 py-3'>
          <div className='space-y-0.5'>
            <FormLabel>{t('Simulate Remote Compact V2')}</FormLabel>
            <FormDescription>
              {t(
  ...
  const proxyImageURLFields = (currentType === 1 || currentType === 24) && (
    <FormField
      control={form.control}
  ...
  const streamTimeoutFields = (
    <FormField
      control={form.control}
      name='stream_first_response_timeout'
      render={({ field }) => (
        <FormItem>
          <FormLabel>{t('Stream first response timeout')}</FormLabel>
          <FormControl>
            <Input
              type='number'
              min={0}
  ...
  const agnesImageFields = currentType === CHANNEL_TYPE_AGNES && (
    <FormField
      control={form.control}
                ...
                <FormField
                  ...
                  render={({ field }) => (
                    <FormItem className='flex items-center justify-between gap-3 px-4 py-3'>
                      <div className='space-y-0.5'>
                        <FormLabel className='text-sm'>
                          {t('Automatically cache Claude stable prefixes')}
                        </FormLabel>
                        <FormDescription>
                          {t(
            ...
            <ChannelAuthSection>
              ...
              {(isMultiKeyChannel ||
                ...
                <MultiKeyReliabilityEditor
                  rules={formValues.multi_key_disable_rules || '[]'}
                  autoRetry={formValues.multi_key_auto_retry === true}
                  autoRecovery={formValues.multi_key_auto_recovery === true}
                  recoveryIntervalMinutes={
                    formValues.multi_key_recovery_interval_minutes || 10
                  }
                  disabled={sensitiveLocked}
                  onRulesChange={(value) =>
                    form.setValue('multi_key_disable_rules', value, {
                      shouldDirty: true,
                      shouldValidate: true,
                    })
                  }
                  ...
                  onAutoRecoveryChange={(value) =>
                    form.setValue('multi_key_auto_recovery', value, {
                      shouldDirty: true,
                      shouldValidate: true,
                    })
                  }
