...
export async function clearSupportData() {
  const res = await api.delete<ClearSupportDataResponse>('/api/option/support')
  return res.data
}

export async function sendSMTPTestEmail(email: string) {
  const res = await api.post<SMTPTestEmailResponse>('/api/option/smtp/test', {
    email,
  })
  return res.data
}
