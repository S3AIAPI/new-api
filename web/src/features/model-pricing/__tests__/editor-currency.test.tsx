...
it('keeps per-token and per-request pricing available as standard modes', () => {
  const editor = renderEditor({
    ratio: '',
    completionRatio: '',
    billingMode: 'per-token',
  })
  expect(screen.getByRole('tab', { name: 'Per-token' })).toHaveAttribute(
    'aria-selected',
    'true'
  )
  expect(screen.getByRole('tab', { name: 'Per-request' })).toBeVisible()
  expect(screen.queryByRole('button', { name: /expression/i })).toBeNull()
