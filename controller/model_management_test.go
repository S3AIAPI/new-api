	...
	for _, dialect := range []struct{ kind, env string }{{"sqlite", ""}, {"mysql", "TEST_MYSQL_DSN"}, {"postgres", "TEST_POSTGRES_DSN"}} {
		t.Run(dialect.kind, func(t *testing.T) {
			...
			t.Run("pricing_saves_zero_switches_modes_and_rejects_stale_batches", func(t *testing.T) {
				previewBefore, err := model.GetModelPricingSnapshot([]string{"matrix-preview"})
				require.NoError(t, err)
				var preview struct {
					Success bool
				...
				modelManagementRequest(t, PreviewModelPricing, http.MethodPost, "/api/option/model_pricing/preview", map[string]any{
					"model_name": "matrix-preview",
					"pricing":    model.PricingValues{"ModelRatio": float64(2), "CreateCacheRatio": float64(1.25)},
				}, &preview)
				require.True(t, preview.Success)
				assert.Equal(t, float64(2), preview.Data.Effective["ModelRatio"])
				assert.Equal(t, model.CacheWriteStandard, preview.Data.CacheWriteMode)
				previewAfter, err := model.GetModelPricingSnapshot([]string{"matrix-preview"})
				require.NoError(t, err)
				assert.Equal(t, previewBefore.Entries, previewAfter.Entries, "preview must not write pricing")
