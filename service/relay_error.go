...
func ShouldRetryRelayError(c *gin.Context, openaiErr *types.NewAPIError, retryTimes int) bool {
	...
	// A specifically selected channel must not fall through to another
	// channel after a relay failure. This context is used by token/channel
	// affinity paths that cannot safely retry elsewhere.
