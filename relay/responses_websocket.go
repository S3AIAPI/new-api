...
type responsesWSCallState struct {
	...
	terminalMessages []*responsesWSMessage
	closeAfter       bool
}
...
func (s *responsesWSSession) runRequest(state *responsesWSCallState, message []byte, requestID string) {
	...
	defer func() {
		...
		outgoing := state.terminalMessages
		if apiErr != nil {
		...
		for _, message := range outgoing {
			...
			if err := s.client.SetWriteDeadline(time.Now().Add(responsesWSWriteTimeout)); err != nil {
				state.closeAfter = true
				break
			} else if err := s.client.WriteMessage(message.kind, message.body); err != nil {
				state.closeAfter = true
				break
			}
		}
	...
	for {
		...
		case incoming := <-state.inbox:
			...
			if incoming.err != nil {
				...
				if failure, err := helper.FinalizeSimulatedRemoteCompactV2WebSocket(c); err != nil {
					info.StreamStatus.RecordError(err.Error())
				} else if len(failure) > 0 {
					state.terminalMessages = []*responsesWSMessage{{kind: websocket.TextMessage, body: failure}}
				}
				state.closeAfter = true
			...
			if err := common.Unmarshal(incoming.body, &event); err != nil {
				...
				handled, simulationTerminal, outputs, simulationErr := helper.HandleSimulatedRemoteCompactV2WebSocketEvent(c, event, incoming.body)
				if simulationErr != nil {
					info.StreamStatus.RecordError(simulationErr.Error())
					state.closeAfter = true
					ConsumeResponsesQuota(c, info, accumulator.Finish())
					return types.NewError(simulationErr, types.ErrorCodeBadResponse, types.ErrOptionWithSkipRetry())
				}
				if handled {
					if simulationTerminal {
						state.terminalMessages = make([]*responsesWSMessage, 0, len(outputs))
						for _, output := range outputs {
							state.terminalMessages = append(state.terminalMessages, &responsesWSMessage{kind: websocket.TextMessage, body: output})
						}
			...
			case "response.completed", "response.done", "response.incomplete", "response.failed", "response.cancelled", "response.canceled":
				...
				info.StreamStatus.SetEndReason(relaycommon.StreamEndReasonDone, nil)
				state.terminalMessages = []*responsesWSMessage{&incoming}
				ConsumeResponsesQuota(c, info, accumulator.Finish())
				return nil
			}
...
func (s *responsesWSSession) restoreConnectionContext(c *gin.Context, model string) *types.NewAPIError {
	...
	if channel.ChannelInfo.IsMultiKey {
		...
		if s.lockedKeyIndex >= 0 && s.lockedKeyIndex < len(keys) {
			status := channel.ChannelInfo.MultiKeyStatusList[s.lockedKeyIndex]
			keyEnabled = keys[s.lockedKeyIndex] == s.lockedKey && (status == 0 || status == common.ChannelStatusEnabled)
		} else {
			keyEnabled = false
		}
	}
